...
class SAC(nn.Module):
    ...
    def _build(self):
        ...
        pc_spec, distance_spec = map(self.observation_spec.get, ('point_cloud', 'distance'))
        frames_stack, pn_number, in_channels = pc_spec.shape
        _, distance_dim = distance_spec.shape
        assert distance_dim == 1
        ...
        # RL
        self.actor = models.Actor(emb + distance_dim*frames_stack, act_dim, self._c.actor_layers)

        self.critic = models.Critic(emb + distance_dim*frames_stack + act_dim, self._c.critic_layers)
